...
var options = {
  //지도를 생성할 때 필요한 기본 옵션
  center: new kakao.maps.LatLng(36.44, 127.96), //지도의 중심좌표. 서울 한가운데
  level: 12, //지도의 레벨(확대, 축소 정도) 3에서 8레벨로 확대
};
...
async function getDataSet(CITY) {
  ...
  const dataSet = await axios({
    ...
    url: `http://43.200.120.171:3000/foodinfo?CITY=${qs}`,
...
function getContent(data) {
  ...
  <div class="infowindow">
      ...
      <div class="infowindow-body">
        ...
        <p class="infowindow-address">${data.ADDR}</p>
        <p class="infowindow-tel">전화번호: ${data.TEL}</p>
        <a href="${data.UTUBE_URL}" class="infowindow-btn" target="_blank">영상이동</a>
      </div>
    </div>
  `;
}
